module.exports = (existing, local) => {
    ...
    //
    // const changed = (a, b) => JSON.stringify(a) !== JSON.stringify(b);
    // const checkOptions = (existingOptions, localOpts) => {
    //     return localOpts.some((localOpt) => {
    //         const existingOpt = existingOptions.find((opt) => opt.name === localOpt.name);
    //         if (!existingOpt) return true;
    //         return hasDifference(localOpt, existingOpt);
    //     })
    // }
    // if (existingName !== localName || existingDescription !== localCommandDescription || checkOptions(existingOptions, localCommandOptions)) {
    //     return true;
    // }
    //
    // return false;
